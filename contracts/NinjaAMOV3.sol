...
contract NinjaToken is ERC20("NinjaSwap", "NINJA"), Ownable {
    ...
    modifier onlyMinter() {
        require(AMO == msg.sender || MasterChef == msg.sender, "No Permission");
        _;
    }
}

// File: contracts/NinjaAMOV2.sol
...
contract NinjaAMOV3 is ReentrancyGuard, Ownable, Pausable {
    using SafeMath for uint256;
    ...
    address public NinjaBountyAirdropBank;

    bool bountyTokensMinted = false;
    ...
    //Vesting rounds
    uint256 public vestingRounds = 1;

    // initial unlocked tokens on deposit
    uint256 public initialUnlockPercent = 50;

    //Vesting period 2629743 = 1 month, 604800 = 1 week
    uint256 public vestingPeriod = 604800;
    ...
    constructor(
        ...
        address _NinjaBountyAirdropBank
    ) public {
        ninjaToken = _ninjaToken;
        BUSD = _busd;
        developmentTreasury = _developmentTreasury;
        marketersTreasury = _marketersTreasury;
        NinjaBountyAirdropBank = _NinjaBountyAirdropBank;
    ...
    function mintWithBNB(address _beneficiary)
        public
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        TotalLockedTeamTokens = TotalLockedTeamTokens.add(_teamShare);
        bnbDeposits[msg.sender] = bnbDeposits[msg.sender].add(bnbAmount);
        _checkout(_beneficiary, tokensToBeMint);
    }

    function mintWithBUSD(uint256 _amountBusd)
        public
        whenNotPaused
        nonReentrant
    {
        ...
        _checkout(msg.sender, tokensToBeMint);
    }
    ...
    function _checkout(address beneficiary, uint256 _amount) internal {
        ...
        ninjaToken.mint(address(beneficiary), initialUnlockAmount);
        ninjaToken.mint(address(this), remainingAmount);
        Claimed[beneficiary] = Claimed[beneficiary].add(initialUnlockAmount);
        Minted[beneficiary] = Minted[beneficiary].add(_amount);
        emit Mint(beneficiary, _amount);
        ...
        TotalAMOV3Mints = TotalAMOV3Mints.add(_amount);
    }
    ...
    function myNextRelease(address vestor)
        external
        view
        returns (Vesting memory vesting)
    {
        Vesting[] memory _vestings = vestings[vestor];
        ...
        returns (uint256)
    {
        ...
        if (amoTotalminted.add(canBeMint.add(_teamShare)) > maxMintable) {
            // Only amo minted allowed 2.8 million
    ...
    function getEstimatedTokensMintWithBNB(uint256 _bnbAmount)
        public
        view
        returns (uint256)
    {
    ...
    function withdrawTeamTokens(address _to, uint256 _amount)
        external
        onlyOwner
    {
        require(
            _amount <= TotalLockedTeamTokens,
            "requested amount is greater then total locked tokens"
        );
        ninjaToken.transfer(_to, _amount);
        TotalLockedTeamTokens = TotalLockedTeamTokens.sub(_amount);
    }

    function mintBountyTokens() external onlyOwner {
        require(!bountyTokensMinted, "Tokens already minted");
        ninjaToken.mint(NinjaBountyAirdropBank, 30000000000000000000000); //mint 30k ninja tokens for Reward of NinjaSwap Round # 2
        bountyTokensMinted = true;
    }
