...
contract NinjaStarter is ReentrancyGuard, Ownable, Pausable {
    ...
    // IDO starting date
    uint256 public startDate = 1639850400;

    // limit on each address on buy
    uint256 public buyCap = 0;

    // offering token price in BUSD
    uint256 public buyPrice = 0.0425 ether;

    // offering token price in BUSD
    uint256 public offeringTokenDecimals = 10**9;
    ...
    //Vesting rounds
    uint256 public vestingRounds = 12;
    ...
    bool public whitelistChecker = false;

    uint256 public NinjaStakingBalanceRequired = 0;
        ...
        checkBuyCondition(msg.value, _beneficiary)
    {
        ...
        if (
            tokensToBePurchased >
            (offeringToken.balanceOf(address(this))).mul(10**9)
        ) {
            tokensToBePurchased = offeringToken.balanceOf(address(this)).mul(
                10**9
            );
        }
        require(
    ...
    function releaseAll() external {
        ...
        offeringToken.safeTransfer(address(msg.sender), available.div(10**9));
    ...
    function setIDOSettings(
        ...
        uint256 _offeringTokenDecimals
    ) public onlyOwner {
        ...
        offeringTokenDecimals = _offeringTokenDecimals;
    }
